...
##Example:
which("4638"==colnames(NormFiltered))
which("4652"==colnames(NormFiltered))
...
OutlierRemovalIndices<-c(-3, -8, -19) 
...
LinearModel3<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+BrainpHNoOutliers+RNAintegrityNoOutliers+HoursFinalNoOutliers)}
...
setwd("~/Documents/AMY LCM 10nuclei/PAC/13 BrainInABlender")
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot: PAC", xlim=c(-3,3), cex.lab=1.3, cex=0.6))
...
hist(LM2SE[,2], breaks=100, col=2, xlab="SE", main="PAC", cex.lab=1.3)
dev.off()
