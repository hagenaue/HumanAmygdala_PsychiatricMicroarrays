...
RawData<-as.matrix(read.csv("Sample Probe Profile_central_rescan.csv", header=T, row.names=1))
...
####A. AgeNoOutliers (PC1)
####B. TODNoOutliers (PC1) (also correlates closely with RNAintegrity)
####C. RNAintegrityNoOutliers (PC1)
####D. GenderNoOutliers (PC3) - not quite significant
####E. BrainpHNoOutliers doesn't relate to any of the PC!  Perhaps because the central amygdala is easily affected by excitotoxicity?
...
#*****To be prudent, I also included a model that contained BrainpH*************

##2. Insert those variables into this code, placing diagnosis as the first variable.

LinearModel2<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+AgeNoOutliers+RNAintegrityNoOutliers+GenderNoOutliers+HoursFinalNoOutliers+BrainpHNoOutliers)}
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot: Central", xlim=c(-3,3), cex.lab=1.3, cex=0.6))
...
hist(LM2SE[,2], breaks=100, col=2, xlab="SE", main="Central", cex.lab=1.3)
dev.off()
...
setwd("~/Documents/AMY LCM 10nuclei/Central/13 BrainInABlender")
