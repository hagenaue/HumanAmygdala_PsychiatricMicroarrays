...
##Example:
which("4235"==colnames(NormFiltered))
which("4839"==colnames(NormFiltered))
...
OutlierRemovalIndices<-c(-1, -6, -14, -20) 
...
LinearModel3<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+GenderNoOutliers+RNAconcNoOutliers+RNAintegrityNoOutliers+HoursFinalNoOutliers)}
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot: CO", xlim=c(-3,3), cex.lab=1.3, cex=0.6))
...
hist(LM2SE[,2], breaks=100, col=2, xlab="SE", main="CO", cex.lab=1.3)
dev.off()
...
setwd("~/Documents/AMY LCM 10nuclei/CO/13 BrainInABlender")
...
SirUnMixALotOutput<-Sir_UnMixALot(userInput=temp, dataColumns=c(2:39), geneColumn=1, species="human")
