...
if(length(RawData[1,])0)==!90){print("ERROR! The data matrix is the wrong size! Fix now or everything else will be wrong!")}else{print("Input matrix is the correct size")}
...
AVG_Signal<-matrix(as.numeric(RawData[, seq(from=2, to=85, by=2)]), nrow=TotalNumberOfProbes, ncol=42)
colnames(AVG_Signal)<-colnames(RawData[, seq(from=2, to=85, by=2)])
...
GeneIdentifiers<-RawData[,c(1, c(86:106))]
...
AVG_Signal2<-t(apply(AVG_Signal, 1, function(y) y+abs(MinimumSignals))+1)
Log.AVG_Signal<-log(AVG_Signal2, 2)
}else{
print("Data can be log transformed: All minimum signals are above 0")
...
##Example:
which("4310"==colnames(NormFiltered))
...
colnames(SimpleTtestOutput)<-c("Unadjusted P-value", "BH Adjusted Pvalue", "BY Adjusted Pvalue", "Fold Change", "Symbol", "Search Key", "ILMN_Gene", "Chromosome", "Definition", "Synonyms", "TargetID", "ProbeID", "SPECIES", "SOURCE_REFERENCE_ID", "REFSEQ_ID", "UNIGENE_ID", "ENTREZ_GENE_ID", "PROTEIN_PRODUCT", "PROBE_TYPE", "PROBE_START", "PROBE_SEQUENCE", "PROBE_CHR_ORIENTATION", "PROBE_COORDINATES", "ONTOLOGY_COMPONENT", "ONTOLOGY_PROCESS", "ONTOLOGY_FUNCTION")   
...
####A. AgeNoOutliers (PC1)
####B. OverdoseNoOutliers (PC2, PC4)
####C. RNAintegrityNoOutliers (PC3)
####D. GenderNoOutliers (PC4)
####E. HoursIceNoOutliers (PC4)
####F. SuicideNoOutliers (PC4)
...
LinearModel2<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+AgeNoOutliers+BrainpHNoOutliers+GenderNoOutliers+HoursFinalNoOutliers)}
...
LinearModel3<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+GenderNoOutliers+RNAconcNoOutliers+RNAintegrityNoOutliers+HoursFinalNoOutliers+BrainpHNoOutliers+RNAAgeNoOutliers)}
...
##4. The results are found in "LM3testOutput.csv."


#************************************************************************************************
##2. Insert those variables into this code, placing diagnosis as the first variable.

LinearModel4<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+AgeNoOutliers+BrainpHNoOutliers+GenderNoOutliers+HoursFinalNoOutliers+SuicideNoOutliers)}
...
#Applying two different types of multiple-comparison corrections to the raw p-values (Benjamini-Hochberg and Benjamini-Yekutieli):
TempPvalAdjLM4<-mt.rawp2adjp(LM4pvalues[,7], proc=c("BH", "BY"))
LM4PvalAdj<-TempPvalAdjLM4$adjp[order(TempPvalAdjLM4$index),]
row.names(LM4PvalAdj)<-row.names(normNoOutliers)

#Outputting the p-values, betas, and gene identifiers:
LM4testOutput2<-cbind(LM4PvalAdj, LM4Betas[,7], GeneIdentifiers_filtered)
row.names(LM4testOutput2)<-row.names(normNoOutliers)
colnames(LM4testOutput2)<-c("Unadjusted P-value", "BH Adjusted Pvalue", "BY Adjusted Pvalue", "Fold Change", "Symbol", "Search Key", "ILMN_Gene", "Chromosome", "Definition", "Synonyms", "TargetID", "ProbeID", "SPECIES", "SOURCE_REFERENCE_ID", "REFSEQ_ID", "UNIGENE_ID", "ENTREZ_GENE_ID", "PROTEIN_PRODUCT", "PROBE_TYPE", "PROBE_START", "PROBE_SEQUENCE", "PROBE_CHR_ORIENTATION", "PROBE_COORDINATES", "ONTOLOGY_COMPONENT", "ONTOLOGY_PROCESS", "ONTOLOGY_FUNCTION")   

write.csv(LM4testOutput2, "LM4testOutputSuicide.csv")
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot: PL", xlim=c(-3,3), cex.lab=1.3, cex=0.6))
...
hist(LM2SE[,2], breaks=100, col=2, xlab="SE", main="PL", cex.lab=1.3)
dev.off()
...
setwd("~/Documents/AMY LCM 10nuclei/PL/13 BrainInABlender")
...
SirUnMixALotOutput<-Sir_UnMixALot(userInput=temp, dataColumns=c(2:42), geneColumn=1, species="human")
...
pdf("Astrocyte_ByDiagnosis.pdf", height=4.5, width=4)
boxplot(AveragePrimary_CellTypeIndex[1,]~DiagnosisNoOutliers, xlab="Diagnosis", ylab="Astrocyte Index", lwd=1.5, cex.lab=1.3)
stripchart(AveragePrimary_CellTypeIndex[1,]~DiagnosisNoOutliers, vertical = TRUE, 
           method = "jitter", add = TRUE, pch = 20, cex=1.5, col = c(3,2))
mtext("AAA", line=1, cex=1.5)
dev.off()


pdf("ImmatureOligo_ByDiagnosis.pdf", height=4.5, width=4)
