...
GeneIdentifiers<-RawData[,c(1, c(127:144))]
...
pdf("PC1_ByDiagnosis.pdf", height=4.5, width=4)
boxplot(PC1noOutliers~DiagnosisNoOutliers, xlab="Diagnosis", ylab="PC1", lwd=1.5, cex.lab=1.3)
stripchart(PC1noOutliers~DiagnosisNoOutliers, vertical = TRUE, 
...
Detection_PvalueMedianCTRL<-apply(Detection_Pvalue_filteredNoOutliers[,DiagnosisNoOutliers=="Control"], 1, median)
...
NormNoOutliersMedianCTRL<-apply(normNoOutliers[,DiagnosisNoOutliers=="Control"], 1, median)
...
###For example, in the L amygdala, these variables are related to diagnosis:
####A. HoursFinalNoOutliers
####B. SuicidenoOutliers (duh!)

###And these variables are related to the PCs:
####A. BrainpHNoOutliers (PC1, PC3)
####B. AgeNoOutliers (PC2)
####C. CohortNoOutliers (PC2)
####D. GenderNoOutliers (PC2)
####E. DiagnosisNoOutliers (PC4)
...
LinearModel2<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+HoursFinalNoOutliers+AgeNoOutliers+GenderNoOutliers+BrainpHNoOutliers+CohortNoOutliers)}
...
LinearModel4<-function(i){lm(normNoOutliers[i,]~DiagnosisNoOutliers+HoursFinalNoOutliers+AgeNoOutliers+GenderNoOutliers+BrainpHNoOutliers+SuicideNoOutliers)}
...
png("19 Model Comparison LM2 vs. SimpleTTest MDD.png")
plot(log(LM2pvalues[,2])~log(SimplePval[,1]), main="Model Comparison: LM2 vs. Simple T-test MDD", xlab="Simple T-test", ylab="LM2")
...
setwd("~/Documents/AMY LCM 10nuclei/Lateral/13 BrainInABlender")
...
sum(GeneIdentifiers_filtered[,1]=="MEIS2")

normNoOutliers[GeneIdentifiers_filtered[,1]=="MEIS2",]

pdf("MEIS2_ByDiagnosis.pdf", height=4.5, width=4)
boxplot(normNoOutliers[GeneIdentifiers_filtered[,1]=="MEIS2",]~DiagnosisNoOutliers, xlab="Diagnosis", ylab="MEIS2", lwd=1.5, cex.lab=1.3)
stripchart(normNoOutliers[GeneIdentifiers_filtered[,1]=="MEIS2",]~DiagnosisNoOutliers, vertical = TRUE, 
           method = "jitter", add = TRUE, pch = 20, cex=1.5, col = c(3,2))
mtext("Lateral", line=1, cex=1.5)
dev.off()
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot: Lateral", xlim=c(-3,3), cex.lab=1.3, cex=0.6))
...
hist(LM2SE[,2], breaks=100, col=2, xlab="SE", main="Lateral", cex.lab=1.3)
dev.off()
